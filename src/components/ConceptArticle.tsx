...
export function ConceptArticle({ location }: ConceptArticleProps) {
  ...
  return (
    <article className="max-w-4xl">
      ...
      <h1 className="mt-3 text-4xl font-semibold tracking-[0.01em] text-chalk-100 sm:text-5xl">{concept.title}</h1>
      ...
      <section className="mt-10 grid gap-5">
        <h2 className="text-2xl font-semibold tracking-[0.01em] text-chalk-100 sm:text-3xl">Explanation</h2>
        {concept.explanation.map((paragraph) => (
          <p key={paragraph} className="text-base leading-8 text-chalk-200/75">
            {paragraph}
          </p>
        ))}
      </section>
      ...
      {concept.exampleCode ? (
        <section className="mt-10 grid gap-5">
          <h2 className="text-2xl font-semibold tracking-[0.01em] text-chalk-100 sm:text-3xl">Example</h2>
      ...
      {concept.commonMistakes ? (
        <section className="mt-10 rounded-lg border border-white/10 bg-ink-900/60 p-5">
          <h2 className="text-2xl font-semibold tracking-[0.01em] text-chalk-100">Common mistakes</h2>
          <ul className="mt-4 grid gap-2 text-sm leading-6 text-chalk-200/75">
            {concept.commonMistakes.map((mistake) => (
              <li key={mistake} className="flex gap-3">
                <span className="mt-2 size-1.5 rounded-full bg-theorem-300/80" />
                <span>{mistake}</span>
              </li>
            ))}
          </ul>
